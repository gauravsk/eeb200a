...
The mean of 10000 randomly drawn results from a binomial distribution with $p = 0.1$ is ~0.1.
...
gene_frequencies <- function(num_generations = 1000, init_freq = 0.1, num_genes = 3, pop_size = 1000) {
...
\textsc{Use the function that you just wrote to simulate drift with the following parameters: N=100, L=1000, T=10000, p=0.1}
...
\textsc{How many of the 1000 SNP are at frequency 0 at the end of the simulation (in generation 10000)? How many are at frequency 1?}
```{r question-3-b-c}
...
\textsc{Make a plot of the allele frequency trajectories for 100 of the SNPs.}
...
\textsc{How do the two values compare?}
...
\textsc{a) Add mutations to the genealogies (really the coalescent times) that you simulated in question 6 (N=20,000) using the algorithm I just outlined}
...
Again, $4*N*\mu = Num SNPs observed$. As the $N$ is smaller here than in the previous question, the lineages on average have fewer SNPs after the same number of generations. 


### Question 11    

\textsc{Make a scatter plot of the number of SNPs seen in each simulation replicate vs. the TMRCA for that simulation replicate}   

```{r question-11-a, cache = T}
par(mfrow = c(1,2))
color <- rgb(0,0,0,alpha=0.1) 
plot(x = coal_times_small, y = num_snps_small, main = "SNPs v TMRCA", 
     xlab = "Time to coalescence", ylab = "Number of SNPs accumulated", pch = 19, col = color)
...
When $N = 10000$, ~10% of the simulations yield >= 10 SNPs. 

\textsc{ Is the model with N=1,000 compatible with your observed data? What is the proportion of simulation replicates that have >= 10 SNPs?}
