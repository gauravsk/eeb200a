...
print("95CI of TSplit when N known"); quantile(q2_8_accepts$t_splits, probs = c(0.025, 0.975))
```

The median and mean estimates of $T_{split}$ are shifted whehn $N_{ancestral}$ is allowed to vary, but the estimates are of the same order of magnitude (the estimates if $N_{anc}$ is unknown tend to be ~10% lower than when $N_{anc} = 100000$.  The same applies to the confidence interval, which is shifted slightly to the left in the second scenario.)
